...
public class DualSaltTest {
    ...
    private void testKeyAddition(byte[] keySeedA, byte[] keySeedB){
        ...
        byte[] pubKeyA = new byte[dualPublicKeyLength];
        byte[] pubKeyB = new byte[dualPublicKeyLength];
        byte[] secKeyA = new byte[dualSecretKeyLength];
        byte[] secKeyB = new byte[dualSecretKeyLength];
        createDualKeyPair(pubKeyA, secKeyA, keySeedA);
        createDualKeyPair(pubKeyB, secKeyB, keySeedB);

        byte[] pubKeyAB1 = addPublicKeyParts(pubKeyA, pubKeyB);

        byte[] secScalarAB = addScalars(secKeyA, secKeyB);
        byte[] secSecAB = new byte[secretKeyLength];
    ...
    private static void testRotateKeys(byte[] keySeedA, byte[] keySeedB, byte[] rotateSeed) {
        ...
        byte[] pubKeyA1 = new byte[dualPublicKeyLength];
        byte[] pubKeyB1 = new byte[dualPublicKeyLength];
        byte[] secKeyA1 = new byte[dualSecretKeyLength];
        byte[] secKeyB1 = new byte[dualSecretKeyLength];
        createDualKeyPair(pubKeyA1, secKeyA1, keySeedA);
        createDualKeyPair(pubKeyB1, secKeyB1, keySeedB);

        byte[] pubKeyAB1 = addPublicKeyParts(pubKeyA1, pubKeyB1);

        byte[] secKeyA2 = rotateKey(secKeyA1, rotateSeed, true);
        byte[] secKeyB2 = rotateKey(secKeyB1, rotateSeed, false);
        byte[] pubKeyA2 = Arrays.copyOfRange(secKeyA2, 64, dualSecretKeyLength);
        byte[] pubKeyB2 = Arrays.copyOfRange(secKeyB2, 64, dualSecretKeyLength);
    ...
    private void testSingleSign(byte[] keySeed, byte[] message){
        ...
        createSingleKeyPair(publicKey, secretKey, keySeed);

        byte[] signature = signCreate(message, secretKey);

        assertTrue("Verified signature failed", signVerify(signature, publicKey));
    }
    ...
    private void testNegativeSingleSign(byte[] keySeed, byte[] message) {
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            assertFalse("Validated succeeded but it should not", signVerify(tempSignature, publicKey));
        }
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            assertFalse("Validated succeeded but it should not", signVerify(signature, tempPublicKey));
        }
    ...
    private void testDualSign(byte[] keySeedA, byte[] keySeedB, byte[] random, byte[] message) {
        ...
        byte[] virtualPublicKey = addPublicKeyParts(pubKeyA, pubKeyB);

        byte[] m1 = signCreateDual1(message, secKeyA, virtualPublicKey, randomA);
        byte[] m2 = signCreateDual2(m1, secKeyB, randomB);
        byte[] signature = signCreateDual3(m1, m2, secKeyA, randomA);

        assertTrue("Verified signature failed", signVerify(signature, virtualPublicKey));
    }
    ...
    private void testNegativeDualSign(byte[] keySeedA, byte[] keySeedB, byte[] random,  byte[] message) {
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            try {
                signCreateDual3(m1, tempM2, secKeyA, randomA);
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            try {
                signCreateDual3(m1, m2, tempSecKeyA, randomA);
                fail("Validated succeeded but it should not");
            } catch (IllegalArgumentException iae) {
                // Do nothing. It shall fail.
            }
    ...
    private void testSingleDecrypt(byte[] keySeed, byte[] keySeedE, byte[] message){
        ...
        byte[] secKey = new byte[secretKeyLength];
        createSingleKeyPair(pubKey, secKey, keySeed);

        byte[] cipherMessage = encrypt(message, pubKey, keySeedE);
        System.out.println( "Cipher message:  " + TweetNaclFast.hexEncodeToString(cipherMessage));

        byte[] decryptedMessage = decrypt(cipherMessage, secKey);
    ...
    private void testNegativeSingleDecrypt(byte[] keySeed, byte[] keySeedE, byte[] message) {
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            try {
                decrypt(tempCipherMessage, secKey);
    ...
    private void testDualDecrypt(byte[] keySeedA, byte[] keySeedB, byte[] keySeedE, byte[] message) {
        ...
        createDualKeyPair(pubKeyA, secKeyA, keySeedA);
        createDualKeyPair(pubKeyB, secKeyB, keySeedB);
        byte[] pubKeyAB = addPublicKeyParts(pubKeyA, pubKeyB);

        byte[] cipherMessage = encrypt(message, pubKeyAB, keySeedE);
        System.out.println( "Cipher message: " + TweetNaclFast.hexEncodeToString(cipherMessage));

        byte[] d1 = decryptDual1(cipherMessage, secKeyA);
        byte[] decryptedMessage = decryptDual2(d1, cipherMessage, secKeyB);
        assertArrayEquals("Decrypt message failed", message, decryptedMessage);
    }
    ...
    private void testNegativeDualDecrypt(byte[] keySeedA, byte[] keySeedB, byte[] keySeedE, byte[] message) {
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            tempCipherMessage[j] = (byte) (tempCipherMessage[j] ^ 0x1);
            try {
                decryptDual2(d1, tempCipherMessage, secKeyB);
                fail("Decryption succeeded but it should not");
            } catch (IllegalArgumentException iae) {
                // Do nothing. It shall fail.
            }
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            try {
                decryptDual2(tempD1, cipherMessage, secKeyB);
                fail("Decryption succeeded but it should not");
            } catch (IllegalArgumentException iae) {
                // Do nothing. It shall fail.
            }
    ...
    private void testBadGroupElement(byte[] badGroupElement) {
        ...
        decryptDual1(addArrays(badGroupElement, new byte[]{0}), dummySecretKey);
    }
    ...
    public void testGoodGroupElement1() {
        ...
        TweetNaclFast.crypto_hash(tempSecretKey, seed, 0, seedLength);
        tempSecretKey[0] &= 248;
